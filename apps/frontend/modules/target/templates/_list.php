      ...
      foreach ($target_lists as $target_list): /* @var $target_list MailingList */
        ...
        <tr>
          ...
          <td>
            ...
            <a class="btn btn-mini" href="<?php echo url_for('target_edit', array('id' => $target_list->getId())) ?>">edit</a>
            <a class="btn btn-mini ajax_link" href="<?php echo url_for('target_copy', array('id' => $target_list->getId())) ?>">copy</a>
            <?php endif ?>
            <?php if (!$tr || !$tr->getActive()): ?>
              <a class="btn btn-mini ajax_link post" data-submit='<?php echo json_encode(array('csrf_token' => $csrf_token_join, 'id' => $target_list->getId())) ?>' href="<?php echo url_for('target_join') ?>">join</a>
            <?php endif ?>
          </td>
        </tr>
      <?php endforeach ?>
