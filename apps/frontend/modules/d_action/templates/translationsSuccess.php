      ...
      <tbody>
        <?php foreach ($translations as $translation): /* @var $translation PetitionText */ ?>
          <tr>
            ...
            <td>
              <a class="btn btn-mini" href="<?php echo url_for('translation_edit', array('id' => $translation->getId())) ?>">edit</a>
              <?php if ($translation->getStatus() == PetitionText::STATUS_ACTIVE): ?>
                <a class="btn btn-mini post ajax_link" data-submit='<?php echo json_encode(array('lang' => $translation->getId())) ?>' href="<?php echo url_for('widget_create', array('id' => $petition->getId())) ?>">create widget</a>
              <?php endif ?>
              <?php if ($petition->getKind() == Petition::KIND_PLEDGE): ?>
                <a class="btn btn-mini" href="<?php echo url_for('translation_edit', array('id' => $translation->getId())) ?>#pledges">edit pledges</a>
                <a target="_blank" class="btn btn-mini" href="<?php echo url_for('pledge_contact_test', array('petition_id' => $petition->getId())) ?>?lang=<?php echo $translation->getLanguageId() ?>">Test pledge page</a>
              <?php endif ?>
              <a class="btn btn-mini" href="<?php echo url_for('translation_create', array('id' => $petition->getId())) ?>?copy=<?php echo $translation->getId() ?>">Copy for new translation</a>
            </td>
          </tr>
        <?php endforeach ?>
      </tbody>
    </table>
    <a class="btn btn-mini" href="<?php echo url_for('translation_create', array('id' => $petition->getId())) ?>">Create new translation</a>
  </div>
</div>
