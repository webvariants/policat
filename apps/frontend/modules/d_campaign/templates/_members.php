  ...
  <?php if (isset($campaign_rights_list)): ?>
    ...
    <?php if ($admin): ?>
      <form class="ajax_form" method="post" action="<?php echo url_for('campaign_members', array('id' => $campaign->getId())) ?>">
        <p>
          <a class="btn btn-sm submit btn-secondary" data-submit='{"method": "member" }'>Member</a>
          <a class="btn btn-sm submit btn-secondary" data-submit='{"method": "admin" }'>Admin</a>
          <a class="btn btn-sm submit btn-danger" data-submit='{"method": "block" }'>block</a>
        </p>
