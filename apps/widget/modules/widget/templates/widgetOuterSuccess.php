...
switch ($kind) {
  ...
  case Petition::KIND_OLD_EMAIL_ACTION:
    $stylings['headline'] = __('Email action');
    break;
  case Petition::KIND_NETWORK_NEWS:
    $stylings['headline'] = __('Network-News');
    break;
  default:
...
<div onclick="<?php echo UtilWidget::getWidgetHereJs($widget['id'], true) ?>" <?php UtilPolicat::style($css, array('font-size' => '0', 'line-height' => '0', 'max-width' => '1000px'), $stylings['width'] == 'auto' ? null : array('max-width' => ($stylings['width'] . 'px')) )?>>
  ...
  UtilPolicat::style($css, array('line-height' => '18px', 'text-transform' => 'uppercase', 'overflow' => 'visible',
      'color' => $stylings['bg_right_color'], 'background-color' => $stylings['title_color'], 'letter-spacing' => '1px'))
  ?>><?php echo $stylings['headline'] ?></p>
  <img alt="" src="<?php echo $keyvisual ?>" <?php UtilPolicat::style($css, array('line-height' => '12px', 'max-width' => '100%')) ?>>
  <p <?php UtilPolicat::style($css, array('text-align' => 'left', 'font-weight' => 'bold', 'line-height' => '12px', 'margin' => '10px')) ?>>
    <?php echo $title ?>
  </p>
  <div <?php
  ...
  <p <?php UtilPolicat::style($css, array('font-size' => '11px', 'line-height' => '11px', 'color' => $stylings['body_color'], 'margin' => '10px')) ?>>
...
<?php echo (file_get_contents(sfConfig::get('sf_web_dir') . DIRECTORY_SEPARATOR . 'js' . DIRECTORY_SEPARATOR . 'policat_widget_outer_opt.js')); ?>
