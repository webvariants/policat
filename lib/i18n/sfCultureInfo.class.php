...
//          )
  ...
  {
    return __DIR__.'/../vendor/lexpress/symfony1/lib/i18n/data/';
  }
    ...
    foreach ($files as $file)
    {
      ...
      if (in_array($filename, $this->dataFiles) == false)
      {
        if ($extra && $culture === $file) {
          array_unshift($this->dataFiles, $extra);
        } else {
          array_unshift($this->dataFiles, $file);
        }

        $data = &$this->getData($filename);
        if ($culture !== 'root' && $culture !== '__ALIAS' && $culture === $file) {
          foreach (self::$extraCountry as $info) {
            if (array_key_exists('data', $info)) {
              $data = array_replace_recursive($data, $info['data']);
            }
            if (!$extra && array_key_exists($culture, $info)) {
              $data = array_replace_recursive($data, $info[$culture]);
            }
            if ($extra && array_key_exists($extra, $info)) {
              $data = array_replace_recursive($data, $info[$extra]);
            }
          }
        }
